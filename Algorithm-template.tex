...
%===============================环境参数===============================%
\geometry{left=1.5cm, right=1.0cm, top=1.9cm, bottom=0.8cm} % 页边距
...
\lstdefinestyle{cpp}{
    ...
    basicstyle  = \normalsize\setstretch{0.83}\ttfamily, % 保持原设置
    commentstyle= \color[RGB]{0,128,0}\itshape,          % 绿色斜体注释
    keywordstyle= \color[RGB]{0,0,255}\bfseries,         % 亮蓝色加粗关键字
    stringstyle = \color[RGB]{163,21,21},                % 深红色字符串
    identifierstyle=\color{black},                      % 标识符黑色
    emph={int, char, double, float, bool, void, size_t, std, vector, string, cout, endl, return, auto, const},
    emphstyle={\color[RGB]{43,145,175}\bfseries},        % 标准库/类型 青蓝色加粗
...
要注意像2sat一样把该连的边都连上(即要把另一组反向边也连上)要注意数组需要开大一点
\begin{lstlisting}
void solve()
{
    map<string,int> mp;
...
例子:给出一些$[l,r]$之间的异或和或者询问区间$[l,r]$异或和 \quad 强制在线
...
例子:默认一开始所有值都是1 \quad $t=1$:$[l,r]$都变为0 \quad $t=2$:$[l,r]$都变为1   求总体的$sum$
...
每种颜色是独立的,单独考虑每一种颜色。考虑容斥,$ans=\frac{n\times (n+1)}{2}-$简单路径没有点颜色为$k$的数量。考虑后者,将颜色为$k$的点删去,结果为$\sum\frac{(sz_i+1)\times sz_i}{2}-cnt_k$。假设一条边两端颜色分别是$c_1,c_2$,则将其在$x\ne c1\wedge x\ne c_2$时加入,利用线段树分治和可撤销并查集快速维护答案。
...
struct Node
{
    int l,r,len,tag;//注意这里用节点i代表i~i+1这一段的长度
}tr[N<<3];//8倍空间
void pushup(int u)
{
...
void modify(int u,int l,int r,int v)
{
    if (l<=tr[u].l&&r>=tr[u].r) tr[u].tag+=v,pushup(u);//注意pushup
    else
    {
        int mid=tr[u].l+tr[u].r>>1;
        if (l<=mid) modify(u<<1,l,r,v);
...
对于每个$a_i$求出其能合法延伸的最左端点和最右端点$L_i$和$R_i$,那么对于$(l,r)$满足$L_i\le l\le i,i\le r\le R_i$的区间都是合法的。

考虑对于每个$a_i$统计区间,可能会有重复。注意到合法区间可以看成二维平面上的点,实际上可以求所有形如$L_i\le l\le i,i\le r\le R_i$的矩形在二维平面上的面积并。
...
考虑按值分裂维护,每次分成三个部分,$[0,c-1],[c,2c],[2c+1,INF]$,对于第一部分不用管,对于第三部分打一个减法标记和次数标记即可,此时第三部分依然全部大于第一部分,可以合并。对第二部分进行暴力重构后插入平衡树。对于第二部分中的数,每个数至少减少为原来的一半,所以每个数最多重构$O(logn)$次。
...
4. Q l r k 询问$(\sum\limits_{i=l}^{r} A[i] \times (i - l + 1) ^ k)$,保证$0 \le k \le 10$

考虑平衡树上的合并 需要合并$[l,mid-1],[mid,mid],[mid+1,r]$三个区间

$\sum\limits_{i=l}^ra[i]\times(i-l+1)^k=\sum\limits_{i=l}^{mid-1}a[i]\times(i-l+1)^k+a[mid]\times(i-mid+1)^k+\sum\limits_{i=mid+1}^ra[i]\times(i-l+1)^k\\ \quad =ans_{l,k}+a[mid]\times (i-mid+1)^k+\sum\limits_{i=mid+1}^ra[i]\times(i-l+1)^k$

前两部分是好处理的,考虑如何计算第三部分,可以利用二项式定理展开

$\sum\limits_{i=mid+1}^ra[i]\times(i-l+1)^k=\sum\limits_{i=mid+1}^ra[i]\times\sum\limits_{j=0}^k\binom{k}{j}(i-mid)^j(mid-l+1)^{k-j}\\ \quad =\sum\limits_{j=0}^k\sum\limits_{i=mid+1}^r(i-mid)^ja[i]\binom{k}{j}(mid-l+1)^{k-j}=\sum\limits_{j=0}^kans_{r,j}\binom{k}{j}(mid-l+1)^{k-j}$
...
\end{lstlisting}
\subsubsection{三维偏序}
在三维空间上有一些点形如$(x_i,y_i,z_i)$,再给出一些查询$(x_1,x_2,y_1,y_2,z_1,z_2)$。求$\sum\limits_{i=1}^n[x_1\le x_i\le x_2,y_1\le y_i\le y_2,z_1\le z_i\le z_2]$
...
先考虑普通猫树,普通猫树是线段树的拓展。线段树将询问区间拆成$log_n$个区间,然后再进行合并。猫树通过增加预处理的时间和空间,将询问区间拆成两个区间,加快了合并效率。
...
2(\textbf{点权多叉重构树}).实际上我们几乎用不到重构树是二叉树这一性质,因此存在更高妙的做法。不妨设题目限制点权最大值。将节点按权值从小到大排序,按序遍历每个点$i$及其所有出边 $i\rightarrow u$。若$u$已经遍历过,则$w_i\ge w_u$,$max(w_i,w_u)$取到$w_i$,此时若 $i,u$不连通则从 $i$向$u$的代表元连边。

上述做法与一般 kruskal 重构树几乎等价:普通重构树中点权相同的虚点,仅有深度最小的有用:按权值从小到大枚举节点相当于对所有边排序,因为边权即 $max(w_i,w_u)$。这样做不用新建虚点,有效减小了常数。

例:IOI2018狼人

给一个n个点,m条边的无向图,再给定Q个询问,每次询问,给定起点s和t,再给定两个限制L,R,要求在1状态下不能经过编号$\le L$的点,在2状态下不能经过编号$\ge R$的点 ,起始状态是1,可以在中途变为2,问s能否到t
...
Boruvka算法的执行流程是:对于当前每个连通块,都找到其伸出去的最小的边,然后进行合并。每次合并,连通块数量至少减半,故合并次数是$O(logn)$的。每次对于所有集合寻找最小边的时间假如是$O(f)$,那么这个算法时间复杂度是$O(f\times logn)$
...
对儿子处理Info的时候,是可以保留一个儿子的Info的,如果保留重儿子的Info,时间复杂度是$O(nlogn)$
...
先枚举循环同构的偏移量 x,那么就是要求每个第一个集合中的串长度为 x 的后缀和第二个串中长度为 x 的前缀匹配,第二个集合中长度为 n − x 的后缀和第一个集合中串长度为 n − x 的前缀匹配。
...
#define lll __int128
const int P=1709;
const lll mod=(9e18L)+32007943;
void init(string s)
{
    p[0]=1;
    int n=s.size()-1;//把" "+s传进来
    for (int i=1;i<=n;i++)  h[i]=((lll)h[i-1]*P+s[i])%mod,p[i]=(lll)p[i-1]*P%mod;
}
int get(int l,int r) {return (h[r]-(lll)h[l-1]*p[r-l+1]%mod+mod)%mod;}
\end{lstlisting}
...
\begin{lstlisting}
int n,sa[N],rk[N],ork[N],buc[N],id[N],height[N];
char s[N];
void get_sa()
{
    int m=1<<7,p=0;
    memset(buc,0,sizeof buc),memset(ork,0,sizeof ork);
    ...
    for (int w=1;  ;m=p,p=0,w<<=1)
    {
        ...
        memset(buc,0,m+1<<3);//如果define long long这里就是3 否则就是2
        memcpy(ork,rk,n+1<<3);
        p=0;
...
有dp方程:$dp_i=\sum\limits_{0\le j < i}dp_j$($s[j+1,i]$是偶回文串)
...
给定一个序列 $a$,要把它分成 $k$ 个子段。每个子段的费用是其中相同元素的对数。求所有子段的费用之和的最小值。$(2\le n\le10^5,2\le k\le min(n,20))$
