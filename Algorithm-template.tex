...
% \usepackage{unicode-math}  % 现代数学字体支持 - 暂时注释以避免兼容性问题

% 代码高亮和颜色
\usepackage{minted}
\usepackage{xcolor}
\usepackage{tcolorbox} % 增强的盒子环境
...
%============================== 专业化页面布局 ==============================
\geometry{
    left=1.2cm, right=1.0cm, 
    top=2.0cm, bottom=1.2cm,
    headheight=19pt, headsep=10pt,
    footskip=15pt
} % 优化页边距布局
\setlength{\columnsep}{8pt}  % 两栏之间的间距
\setlength\columnseprule{0.5pt} % 加粗分割线

% ============================== 专业化标题样式 ==============================
\usepackage{titlesec}
\usepackage{titletoc}  % 目录样式控制

% 现代化标题设计
\titleformat{\section}
  {\Large\bfseries\sffamily\color{blue!80!black}}
  {\thesection}{1em}{}
\titleformat{\subsection}
  {\large\bfseries\sffamily\color{blue!60!black}}
  {\thesubsection}{0.8em}{}
\titleformat{\subsubsection}
  {\normalsize\bfseries\sffamily\color{blue!40!black}}
...
\setlength{\parindent}{0pt}  % 无段落缩进，现代化排版

% 优化代码块间距 - 适度保留
\setlength{\topsep}{1pt}   % 环境前后间距 - 保留少量
\setlength{\partopsep}{1pt} % 段落前环境间距 - 保留少量

% 进一步压缩算法块之间的间距
\BeforeBeginEnvironment{minted}{\vspace{-4pt}}
\AfterEndEnvironment{minted}{\vspace{-4pt}}

% 添加专门的算法间距压缩
\newcommand{\algosep}{\vspace{-4pt}} % 算法之间的分隔符

% 优化代码块间距
\setlength{\fboxsep}{2pt}  % 代码框内边距
\setlength{\abovecaptionskip}{1pt}  % 标题上方间距 - 最小化
\setlength{\belowcaptionskip}{1pt}  % 标题下方间距 - 最小化
%===============================环境参数===============================%
% ============================== 现代化链接样式 ==============================
\hypersetup{
    colorlinks=true,
...
% 现代化页眉设计
\lhead{\sffamily\footnotesize\color{blue!70!black}\textbf{SCUTSKY}}
\chead{\sffamily\footnotesize\color{gray!80}ACM-ICPC Algorithm Template}
\rhead{\sffamily\footnotesize\color{blue!70!black}Page \textbf{\thepage}}

% 页眉装饰线
\renewcommand{\headrulewidth}{1pt}
\renewcommand{\headrule}{\hbox to\headwidth{%
  \color{blue!20}\leaders\hrule height \headrulewidth\hfill}}
\renewcommand{\footrulewidth}{0pt}

%============================== 专业化字体系统 ==============================
% 现代化中文字体设置
\setCJKmainfont{PingFang SC}[
    UprightFont = PingFang SC Regular,
...
\setCJKmonofont{PingFang SC}  % 保持兼容性

% 专业代码字体配置 - 使用Fira Code
...
% ============================== 专业化代码配置 ==============================
% 配置 minted Python 路径
\renewcommand{\MintedPython}{/opt/homebrew/bin/python3}

% 定义专业配色方案
\definecolor{codebg}{RGB}{255, 255, 255}      % 白色背景
\definecolor{linenum}{RGB}{108, 117, 125}     % 行号颜色
\definecolor{rulecolor}{RGB}{206, 212, 218}   % 边框颜色
\definecolor{keyword}{RGB}{215, 58, 73}       % 关键字红色
\definecolor{string}{RGB}{3, 102, 214}        % 字符串蓝色
\definecolor{comment}{RGB}{106, 115, 125}     % 注释灰色
\definecolor{number}{RGB}{0, 92, 197}         % 数字蓝色
...
% 现代化语法高亮颜色（VS Code风格）
\definecolor{vscode_blue}{RGB}{0, 122, 204}      % VS Code蓝
\definecolor{vscode_purple}{RGB}{175, 0, 219}    % VS Code紫
\definecolor{vscode_green}{RGB}{0, 128, 0}       % VS Code绿
\definecolor{vscode_red}{RGB}{215, 58, 73}       % VS Code红
\definecolor{vscode_orange}{RGB}{255, 140, 0}    % VS Code橙
\definecolor{vscode_gray}{RGB}{96, 139, 78}      % VS Code灰绿
...
\newcommand{\makecover}{
    \begin{titlepage}
        ...
        % 主标题
        {\Huge\sffamily\bfseries\color{blue!80!black}
        ACM-ICPC\\[0.5cm]
        Algorithm Template\par}
        
        \vspace{1.5cm}
        
        % 副标题
        {\Large\sffamily\color{gray!70}
        Competitive Programming Reference\par}
        
        \vspace{2cm}
        
        % 装饰线
        {\color{blue!30}\rule{10cm}{1pt}\par}
        
        \vspace{1cm}
        
        % 作者信息
        {\large\sffamily\color{blue!70!black}
        \textbf{SCUTSKY}\\[0.3cm]
...
% 专业化目录设计
\pagestyle{empty}
\renewcommand{\contentsname}{\sffamily\Large\color{black}目录 Contents}
...
\titlecontents{subsection}[1.5em]
    {\sffamily\color{gray!70}}
    {\thecontentslabel\quad}
    {}
    {\titlerule*[0.6em]{.}\contentspage}
    
\titlecontents{subsubsection}[3em]
    {\sffamily\small\color{gray!50}}
    {\thecontentslabel\quad}
    {}
    {\titlerule*[0.6em]{.}\contentspage}
...
void solve()
{
    ...
    while (k--)
    {
        ...
        {
            if (find(x) == find(y + n) || find(x + n) == find(y)) res++; // 有吃的关系就是假
        ...
        {
            if (find(x) == find(y) || find(x + n) == find(y)) res++; // 有同类或y吃x就是假
...
每种颜色是独立的，单独考虑每一种颜色。考虑容斥，$\text{ans}=\frac{n\times (n+1)}{2}-$简单路径没有点颜色为$k$的数量。考虑后者，
...
stag的值。下推标记就和普通吉司机树一样正常推即可。复杂度$O(n \log^2 n)$
...
{
    if (a > b || st[cur].mx <= x || !st[cur].cnt) return;
    if (a <= l && r <= b && x > st[cur].se)
    {
...
{
    if (a > r || b < l || !st[cur].cnt) return 0;
    if (a <= l && r <= b) return st[cur].cnt + st[cur].scnt;
    int mid = (l + r) >> 1;
    pushdown(cur);
...
考虑按值分裂维护，每次分成三个部分，$[0, c-1], [c, 2c], [2c+1, \infty]$，对于第一部分不用管，
对于第三部分打一个减法标记和次数标记即可，此时第三部分依然全部大于第一部分，可以合并。对第二部分进行暴力重构后插入平衡树。
对于第二部分中的数，每个数至少减少为原来的一半，所以每个数最多重构 $O(\log n)$ 次。
...
$\sum\limits_{i=l}^{r}a[i]\times{(i-l+1)}^k=\sum\limits_{i=l}^{mid-1}a[i]\times{(i-l+1)}^k+
a[mid]\times{(i-mid+1)}^k+\sum\limits_{i=mid+1}^{r}a[i]\times{(i-l+1)}^k\\ \quad =\text{ans}_{l, k}+a[mid]\times
{(i-mid+1)}^k+\sum\limits_{i=mid+1}^{r}a[i]\times{(i-l+1)}^k$

前两部分是好处理的，考虑如何计算第三部分，可以利用二项式定理展开

$\sum\limits_{i=mid+1}^{r}a[i]\times{(i-l+1)}^k=\sum\limits_{i=mid+1}^{r}a[i]\times
\sum\limits_{j=0}^k\binom{k}{j}{(i-mid)}^j{(mid-l+1)}^{k-j}
\\ \quad =\sum\limits_{j=0}^k\sum\limits_{i=mid+1}^{r}{(i-mid)}^{j}a[i]\binom{k}{j}{(mid-l+1)}^{k-j}
=\sum\limits_{j=0}^k\text{ans}_{r, j}\binom{k}{j}{(mid-l+1)}^{k-j}$
...
struct LCT
{
    ...
    void link(int x, int y) { maker(x); if (findr(y) != x) tr[x].fa = y; }
    void cut(int x)//cut的时候不能makeroo
    {
        access(x), splay(x);
...
先考虑普通猫树,普通猫树是线段树的拓展。线段树将询问区间拆成$\log n$个区间,然后再进行合并。
...
Boruvka 算法的执行流程是：对于当前每个连通块，都找到其伸出去的最小的边，然后进行合并。
每次合并，连通块数量至少减半，故合并次数是 $O(\log n)$ 的。每次对于所有集合寻找最小边的时间假如是 $O(f)$，
...
\textbf{例子：}给你一些无向边,边是有颜色和权值的,要求最小化选出最大的边的权值
...
void solve()
{
    ...
    int tot = 0;
    for (int i = 1; i <= n; i++)//这里是操作所有点 包括s和t 一定要注意有没有包含s,
        if (C[i] > 0) add(S, i, C[i]), tot += C[i];
        else if (C[i] < 0) add(i, T, -C[i]);
    add(t, s, INF);
...
$
val(x)=(v_{c_1}+d(c_1, x)) \oplus (v_{c_2}+d(c_2, x)) \oplus \ldots \oplus (v_{c_k}+d(c_k, x))
$
...
\textbf{例子：}求根使得 $\sum\limits_{i=1}^n{dist_{i, root}\times a_i}$最大
...
给出 $N$,以及 $A_{1\ldots N}, B_{1\ldots N}$。对于每个 $k\in [1, N]$,找出一个 $1\ldots N$ 的集合 $S$,满足 $|S |= k$ 且 $\sum\limits_{i\in S}A_i-\max\limits_{i\in S}B_i$ 最大,输出这个值。
...
给定一个序列 $a$，要把它分成 $k$ 个子段。每个子段的费用是其中相同元素的对数。求所有子段的费用之和的最小值。$(2\leqslant n\leqslant 10^5, 2\leqslant k\leqslant \min(n, 20))$
...
struct Pollard_Rho
{
    int qmi_mod(i128 x, int n, int mod)
    {
        int res = 1;
        while (n)
        {
...
{
    using u64 = uint64_t;
    static const int BitNum = 64;
    vector<u64> data; int size, n; u64 mask;
    
    Bitset(int _n = 0) : n(_n)
    {
        size = (n >> 6) + ((n & 63) ? 1 : 0);
        data.assign(size, 0);
        int rem = n & 63;
        mask = rem ? ((1ull << rem) - 1) : ~0ull;
    }
    void reset() { fill(data.begin(), data.end(), 0); }
    void set(int pos, bool val = true)
    {
        int idx = size - (pos >> 6) - 1, bit = pos & 63;
        if (val) data[idx] |= (1ull << bit);
        else data[idx] &= ~(1ull << bit);
    }
    bool test(int pos) const
    {
        int idx = size - (pos >> 6) - 1, bit = pos & 63;
        return data[idx] & (1ull << bit);
    }
    Bitset& operator<<=(int v)
    {
        if (v < 0) { *this >>= -v; return *this; }
        int whole = v >> 6; v &= 63;
        for (int i = 0; i + whole < size; i++) data[i] = data[i + whole];
        for (int i = size - whole; i < size; i++) data[i] = 0;
        if (v > 0)
        {
            u64 carry = 0;
            for (int i = size - 1; i >= 0; i--)
            {
                u64 new_carry = data[i] >> (BitNum - v);
                data[i] = (data[i] << v) | carry;
                carry = new_carry;
            }
        }
        data[0] &= mask; return *this;
    }
    Bitset& operator>>=(int v)
    {
        if (v < 0) { *this <<= -v; return *this; }
        int whole = v >> 6; v &= 63;
        for (int i = size - 1; i >= whole; i--) data[i] = data[i - whole];
        for (int i = whole - 1; i >= 0; i--) data[i] = 0;
        if (v > 0)
        {
            u64 carry = 0;
            for (int i = 0; i < size; i++)
            {
                u64 new_carry = (data[i] & ((1ull << v) - 1)) << (BitNum - v);
                data[i] = (data[i] >> v) | carry;
                carry = new_carry;
            }
        }
    ...
    Bitset operator~() const
    {
        ...
        result.data[0] &= mask; return result;
    }
    bool any() const { for (int i = 0; i < size; i++) if (data[i]) return true; return false; }
    bool none() const { return !any(); }
    int count() const
    {
        int cnt = 0;
        for (int i = 0; i < size; i++) cnt += __builtin_popcountll(data[i]);
        return cnt;
    }
    void flip() { *this = ~(*this); }
    void flip(int pos) { set(pos, !test(pos)); }
};
\end{minted}
\subsection{vector}
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{vuqa.png}
\end{figure}
