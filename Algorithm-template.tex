...
% 空间复杂度：$O(n\log n)$
\begin{minted}{cpp}
int st[N][M], w[N], n; // 使用 ST 表时一定要记得调用 build
void build()
{
...
bitset<N> bs; 声明一个长度为 N 的 bitset
bs.set() bs全部变为1
bs.reset() bs全部变为0
bs.set(x) x这一位变为1
bs.reset(x) x这一位变为0
...
\item \texttt{File \textrightarrow{} Preferences \textrightarrow{} Keyboard Shortcuts}，删除 F11 快捷键，将 F5 的功能改为 F11
\item Windows 系统：右键终端标题栏 \textrightarrow{} 设置 \textrightarrow{} 默认值 \textrightarrow{} 高级 \textrightarrow{} 关闭行为 \textrightarrow{} 从不自动关闭
...
\item 打开 VSCode，点击侧边栏第一个图标 \textrightarrow{} \texttt{Open Folder} \textrightarrow{} \texttt{Desktop/xcpc} \textrightarrow{} \texttt{Open} \textrightarrow{} 选择信任
\item 展开文件夹，新建 \texttt{a.cpp} 文件
\item \texttt{File \textrightarrow{} Auto Save}，启用自动保存
\item \texttt{File \textrightarrow{} Preferences \textrightarrow{} Keyboard Shortcuts}，搜索 \texttt{cph}，找到 \texttt{run-testcases}，改为 F11
